...
check_ghcr_login() {
    ...
    if ! docker info | grep -q "Username.*${GITHUB_USERNAME}"; then
        ...
        echo "   - Create token with 'write:packages' and 'read:packages' permissions"
        echo ""
        echo "2. Login with this command:"
...
build_and_push() {
    ...
    echo "🚀 Next Steps:"
    echo ""
    echo "1. Test deployment (optional):"
    echo "   ./deploy.sh run"
    echo ""
    echo "2. Share with users:"
    echo "   - Tell them to run: ./setup.sh"
    echo "   - Or share these commands:"
    echo "     export GITHUB_USERNAME=${GITHUB_USERNAME}"
    echo "     ./deploy.sh run"
    echo ""
    echo "3. Users will need:"
    echo "   - Docker and Docker Compose"
    echo "   - Your GitHub packages to be public OR"
    echo "   - Access to your private packages"
    echo ""
    echo "📚 Documentation:"
    echo "   - DEPLOY.md - Complete deployment guide"
    echo "   - RUN.md - User run instructions"
}
...
show_help() {
    ...
    echo ""
    echo "🚀 COMMANDS:"
    echo "  build    Build and push images to GHCR (DEVELOPERS ONLY)"
    echo "  run      Pull and deploy from GHCR (FOR ALL USERS)"
    ...
    echo ""
    echo "👥 WHO USES WHAT:"
    echo "  👤 Most Users: Just run './deploy.sh run'"
    echo "  👨‍💻 Developers: Run './deploy.sh build' then './deploy.sh run'"
    echo ""
    echo "📋 EXAMPLES:"
